...
public class FileHandler {
   ...
   /* public void loadPortFile(Map map) throws NullPointerException, FileNotFoundException{
        Dock tempDock; //holds the current dock read in
        Cargo tempCargo; //holds the current cargo read in
        Scanner fileInput; //reads input from the file
        ...
        try{
            fileInput = new Scanner(this.portFile);
                        
            if(fileInput.hasNextLine()){
               currentLine = fileInput.nextLine();
               item = currentLine.split(",");
               map.getPort().setName(item[0]);

               dockAmount = Integer.valueOf(item[1]);
               cargoAmount = Integer.valueOf(item[2]); 
            }
            
            
            for(int i = 0; i < dockAmount && fileInput.hasNextLine(); i++){
                tempDock = new Dock();
                currentLine = fileInput.nextLine();
                item = currentLine.split(",");
                
                tempDock.setName(item[0]);
                tempDock.setSection(item[1].charAt(0));
                tempDock.setNumber(Integer.valueOf(item[2]));
                tempDock.setLength(Double.valueOf(item[3]));
                tempDock.setWidth(Double.valueOf(item[4]));
                tempDock.setDepth(Double.valueOf(item[5]));
                tempDock.setLongitude(Double.valueOf(item[6]));
                tempDock.setLatitude(Double.valueOf(item[7]));                
               
                map.getPort().getDock().add(tempDock);
            }
            
            for(int j = 0; j < cargoAmount && fileInput.hasNextLine(); j++){
                tempCargo = new Cargo();
                currentLine = fileInput.nextLine();
                item = currentLine.split(",");
                
                tempCargo.setDescription(item[0]);
                tempCargo.setWeight(Double.valueOf(item[1]));
