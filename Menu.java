...
public class Menu {
    ...
    public void studentID() { //displays the student ID info 
        System.out.println("-------------------");
        System.out.println("Name: Zach Niemann");
        System.out.println("ID: 1000625866");
        System.out.println("CSE 1325-002");
        System.out.println("March 22, 2015");
        System.out.println("-------------------\n\n");
    }
    ...
    */
    ...
    /*
     * Used to output the current status of the cargo, ship, and dock objects.
     * Also outputs the status and location of the ship 
    */
    public void statusReport(Cargo cargo, CargoShip ship, Dock dock) {
        System.out.println("\nSTATUS REPORT");
        System.out.println("-----------------");
        ...
        if(cargo.getWeight() > ship.getCapacity()) { //max weight exceeded
            System.out.println("* Unsafe due to cargo capacity being exceeded.");
        }
        
        //if the ship is not in the dock, then the ship is at sea.
        if(ship.getLongitude() != dock.getLongitude() || ship.getLatitude() != dock.getLatitude()) {
            System.out.println("* The ship is at sea.");
        }
        
        else { //the ship is at the dock
            System.out.println("* The ship is at the dock.");
            
            //If the ship is larger than the dock, and is in the dock, the report will say the ship is grounded.
            if(ship.getLength() > dock.getLength() || ship.getBeam() > dock.getWidth() || ship.getDraft() > dock.getDepth()) {
                System.out.println("* Grounded due to the ship being larger than the dock.");
            }
            
            //if the ship is safely in the dock, then the ship should be ready for offloading
            else { 
                System.out.println("* Ship is ready for offloading");
            }
        }
        System.out.println("------------------------------------------------------\n");
    }
    ...
    /*
     * Used to navigate the user to the correct menu. All objects are passed
     * since all of them are used through this menu
    */
